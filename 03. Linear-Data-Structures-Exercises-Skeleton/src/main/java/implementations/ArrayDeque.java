...
public class ArrayDeque<E> implements Deque<E> {
    ...
    public void insert(int index, E element) {
        ...
        this.ensureValidIndex(realIndex);
    ...
    public E remove(int index) {
        //our elements are in the middle of array
        int realIndex = this.head + index;
        ensureValidIndex(realIndex);
    ...
    private void  ensureValidIndex(int realIndex) {
        if (realIndex < this.head || realIndex > this.tail) {
            throw new IndexOutOfBoundsException("Invalid index" + (realIndex - this.head));
        }
    }
    ...
    public void trimToSize() {
        ...
        for (int i = this.head; i <= this.tail; i++) {
            trimmedElements[idx++] = this.elements[i];
        }
    ...
    private Object[] resize() {
        ...
        int begin = middle - this.elements.length / 2;

        int idx = this.head;

        for (int i = begin ; idx <= this.tail; i++) {
            newElements[i] = this.elements[idx++];
        }
